...
- **Motion budget: two orchestrated moments** — process bars (scroll-in)
  ...
  prefers-reduced-motion. And the hero's laptop theatre became a
  CLICK TOY (28 Aug, owner: the headline owns the first beat): nothing
  auto-runs — the laptop rests open and lit; clicking it plays the
  handoff (the site swells out of the screen into the panel) and
  clicking the panel returns it. User-triggered motion sits outside the
  motion budget; reduced motion and stacked layouts show the panel
  plainly with no toy. Those carve-outs extend no further.
