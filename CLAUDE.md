...
- **Amber budget:** exactly six at-rest appearances — hero ◆ + ●, the two
  process chips, the directions-artefact ◆ on the demo frame, the fault-list
  payoff rule. The form success line is a transient state, outside the count.
...
- **IO/rAF fallbacks are load-bearing.** Reveals arm only below the fold and
  ...
  jump-to-target backstop. Browsers that don't composite (embedded panes,
  background tabs) never fire IO or rAF — don't remove these.
